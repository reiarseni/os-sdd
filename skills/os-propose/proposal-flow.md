...
Contract shared by `os-propose` and `os-propose-grill`: everything both do
around the interview — the gates before it and the artifacts after it. The
interview itself is the only part that differs, and each skill defines its
own.

## Before the interview

In order, before asking the first question.
...
Run `openspec list --json` (active changes), `openspec status --all --json`
(each active change's `isPlanningComplete` and task counts) and
`openspec list --specs` (capabilities the project already covers):
...
In order, once the skill's own stopping condition is met.

### TDD question

Ask the "TDD question" from `session-options.md`.
